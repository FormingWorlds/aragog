...
def run_dirichlet_test():
    ...
    def rhs_cond(t, S, _s=state_cond, _m=mesh_cond, _si=S_in_bc, _so=S_out_bc):
        _s.dSdt(t, S)
        _s.heat_flux[0] = 0.0
        _s.heat_flux[-1] = 0.0
    ...
    sol_cond = solve_ivp(rhs_cond, (0, 1e8), S_init, method='BDF', atol=0.01, rtol=1e-8)
    ...
    mesh_conv.mixing_length = np.maximum(
        np.minimum(mesh_conv.r_basic - R_INNER, R_OUTER - mesh_conv.r_basic), 1.0
    )
    state_conv = ConstPropState(mesh_conv, viscosity=1e12, convection=True, alpha=1e-5)

    def rhs_conv(t, S, _s=state_conv, _m=mesh_conv, _si=S_in_bc, _so=S_out_bc):
        _s.dSdt(t, S)
        _s.heat_flux[0] = 0.0
        _s.heat_flux[-1] = 0.0
        ef = _s.heat_flux * _m.area
    ...
    sol_conv = solve_ivp(rhs_conv, (0, 1e6), S_init, method='BDF', atol=0.01, rtol=1e-6)
    ...
    return (mesh_cond.r_stag / 1e3, T_cond, T_conv, r_an / 1e3, T_an)
...
def run_neumann_test():
    ...
    t_end = 1e4  # yr
    sol = solve_ivp(
        rhs, (0, t_end), S_init, method='BDF', atol=0.01, rtol=1e-8, dense_output=True
    )
...
def run_mixed_test():
    ...
    def rhs(t, S, _s=state, _m=mesh, _si=S_inner):
        ...
        T_surf = S_to_T(S[-1])
        _s.heat_flux[-1] = emissivity * Stefan_Boltzmann * (T_surf**4 - T_eq**4)
    ...
    t_end = 1e6  # yr
    sol = solve_ivp(
        rhs, (0, t_end), S_init, method='BDF', atol=0.01, rtol=1e-6, dense_output=True
    )
...
def main():
    ...
    ax = axes[0]
    ax.plot(r_an_km, T_an, '--', color=C_COND, linewidth=2.5, label='Conduction ($A/r + B$)')
    ax.plot(
        r_km,
        T_cond,
        '-',
    ...
    # -- Panel (b): Neumann BCs --------------------------------------------
    ax = axes[1]
    ax.plot(times_yr, E_num, '-', color=C_ENERGY, linewidth=2.0, label='$E(t)$ (numerical)')
    ax.plot(
        times_yr,
        E_exp,
        '--',
    ...
    for i, (t, T_t) in enumerate(profiles.items()):
        ...
        ax.plot(r_km_mixed, T_t, '-', color=colors_mixed[i], linewidth=2.0, label=lbl)
    # Mark the fixed CMB temperature
    ax.axhline(4000.0, color='gray', ls=':', lw=1, alpha=0.5)
    ax.text(
        r_km_mixed[0] + 20, 4000 + 40, '$T_{\\mathrm{CMB}}$ = 4000 K', fontsize=8, color='gray'
    )
    ax.set_xlabel('Radius [km]')
    ax.set_ylabel('Temperature [K]')
    ...
    fig.suptitle(
        'BC regime verification: constant-property entropy solver', fontsize=15, y=1.01
    )
    fig.tight_layout()
    ...
    print(
        f'      Convective interior DT spread: {T_spread:.0f} K '
    ...
    print(
        f'  (c) Final T_CMB = {T_cmb_final:.0f} K (target: 4000 K, '
        f'drift: {abs(T_cmb_final - 4000):.0f} K)'
    )
    print(f'      Final T_surf = {T_surf_final:.0f} K (cooled from initial)')
    print('=' * 60)
