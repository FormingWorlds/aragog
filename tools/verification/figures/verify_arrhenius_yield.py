...
def main():
    ...
    ax.semilogx(
        eta_eff_mobile, depth, label='Mobile Lid (Low Yield Stress)', color=c1, linestyle='--'
    )
    ...
    fig_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), '../../../docs/figures/vv')
    )
    os.makedirs(fig_dir, exist_ok=True)

    data_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), '../../../output/aragog_vv_data')
    )
    ...
    try:
        ...
        fig.savefig(
            os.path.join(fig_dir, 'arrhenius_yielding.png'), dpi=300, bbox_inches='tight'
        )
        plt.close(fig)

    npz_path = os.path.join(data_dir, 'arrhenius_yielding.npz')
    np.savez(
        npz_path, depth=depth, eta_eff_stagnant=eta_eff_stagnant, eta_eff_mobile=eta_eff_mobile
    )
