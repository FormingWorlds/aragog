...
"""
    ...
    result = subprocess.run(
        cmd, env=env, capture_output=True, text=True, cwd=str(SPIDER_DIR), timeout=600
    )
...
def main():
    ...
    # (a) T_magma(t)
    ax = axes[0, 0]
    ax.plot(spider['times'], spider['T_magma'], 'b-', linewidth=2, label='SPIDER')
    ax.plot(aragog['times'], aragog['T_magma'], 'r--', linewidth=2, label='Aragog (entropy)')
    ...
    # (b) Phi_global(t)
    ax = axes[0, 1]
    ax.plot(spider['times'], spider['phi_global'], 'b-', linewidth=2, label='SPIDER')
    ax.plot(aragog['times'], aragog['phi_global'], 'r--', linewidth=2, label='Aragog (entropy)')
    ax.set_xlabel('Time [yr]')
    ...
    # (c) F_surf(t)
    ax = axes[1, 0]
    ax.semilogy(spider['times'][1:], spider['F_surf'][1:], 'b-', linewidth=2, label='SPIDER')
    ax.semilogy(
        aragog['times'][1:], aragog['F_surf'][1:], 'r--', linewidth=2, label='Aragog (entropy)'
    )
    ax.set_xlabel('Time [yr]')
    ...
    if len(t_common) > 2:
        f_T_a = interp1d(
            aragog['times'], aragog['T_magma'], bounds_error=False, fill_value='extrapolate'
        )
        f_phi_a = interp1d(
            aragog['times'], aragog['phi_global'], bounds_error=False, fill_value='extrapolate'
        )
    ...
    fig.suptitle('SPIDER vs Aragog parity: standalone grey-body cooling', fontsize=15, y=1.005)
    fig.tight_layout()
    ...
    if len(t_common) > 2:
        ...
        print(
            f'  Max |dPhi/Phi|: {np.max(rel_phi[phi_spider > 0.01]):.1f}%'
            if np.any(phi_spider > 0.01)
            else '  Phi too small to compare'
        )
    ...
    # Save data
    np.savez(
        OUT_DIR / 'spider_parity_data.npz',
        spider_times=spider['times'],
        spider_T=spider['T_magma'],
        spider_phi=spider['phi_global'],
        spider_F=spider['F_surf'],
        aragog_times=aragog['times'],
        aragog_T=aragog['T_magma'],
        aragog_phi=aragog['phi_global'],
        aragog_F=aragog['F_surf'],
    )
