...
class TestEnergyBalanceCoreBC:
    ...
    to check the energy-balance BC contract, not re-test the mesh / flux
    machinery already covered by ``TestJgravSmoothing`` and
    ``TestEntropySolverStandalone``.
    """
    ...
    def _make_bare_solver(entropy_eos, n_stag: int = 30,
        ...
        Uses ``__new__`` + attribute injection, then wires a minimal
        evaluator with an Earth-like mesh so ``set_initial_entropy``'s
        cold-start FD stencil has real r_basic/r_stag arrays to work
        with. Additional cached constants for ``_energy_balance_rhs_per_s``
        and BC dispatch are wired by ``_wire_cached_constants``.
        """
